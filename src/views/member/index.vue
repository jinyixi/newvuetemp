<template>
    ...
    <el-table :data="list" height="580" border style="width: 100%">
        ...
        <el-table-column prop="member_card" label="会员卡号"></el-table-column>
        <el-table-column prop="member_name" label="会员姓名"></el-table-column>
        <el-table-column prop="member_birthday" label="会员生日"></el-table-column>
    ...
    methods: {
        fetchData(){
            ...
            memberApi.getList(token,this.pageSize,this.currenPage).then( response=>{
                this.list = response.data// 将返回数据的data赋值给list
                this.total = response.total  // 将接口返回的total 覆盖 data里的total
            })
      ...
      handleEdit(id) {
        ...
        // 获取token
        const token = localStorage.getItem('token')
        memberApi.getById(id,token).then(res => {
          if (res.success) {
      ...
      handleDelete(id) {
        console.log('删除', id)
              this.$confirm('该操作将永久性删除数据，请谨慎操作。', '提示', {
                  ...
                  // 获取token
                  const token = localStorage.getItem('token')
                  memberApi.deleteById(id,token).then(res => {
                      // 删除成功或失败的提示信息
                      this.$message({
                          message: res.msg,
    ...
    addData(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          memberApi.add(this.pojo,token).then(res => {
              console.log('新增的返回',res);
            if (res.success) {
              // 新增成功，提示语
              this.$message({
                message: res.msg,
      ...
      updateData(formName) {
          ...
          this.$refs[formName].validate(valid => {
            if (valid) {
              const token = localStorage.getItem('token')
              // 提交更新
              memberApi.update(this.pojo,token).then(res => {
                if (res.success) {
                  // 刷新列表
                  this.fetchData();
